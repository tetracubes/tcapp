...
import * as PaymentModeValidations from "./PaymentMode-validator";
...
const PaymentMode = (props) => {
    ...
    const loadData = async () => {
        ...
        await PaymentModeService.fetchAll().then((res) => {
            if (res.data) {
                ...
                let msg = "Payment Mode Data Loaded Successfully!!!" + `( ${utility.formatTime((performance.now() - t0).toFixed(2))} ms)`;
    ...
    const dataStruct = {
        fields: {
            paymentModeId: {
                name: "paymentModeId",
                dbname: "paymentModeId",
                componentType: "View",
                labelDisplayName: "Payment Mode Id: ",

                // required: true
            },
            paymentModeName: {
                name: "paymentModeName",
                dbname: "paymentModeName",
                componentType: "INPUT",
                labelDisplayName: "Payment Mode Name: ",
                defvalue: "",
                required: true
            },
            inActive: {
    ...
    const getFiledValidated = (key, value) => {
        ...
        let rules = PaymentModeValidations.getFieldRules(key, popUpData) || {};
    ...
    const handleSave = () => {
        if (validation()) {
            if (popUpMode == "Add" || popUpMode == "Edit") {
                let data = popUpData;
                // data["paymentModeId"]=-1
                if (popUpMode == "Add") delete data["paymentModeId"]
                ...
                // data["inActive"]="N";
                props.appConfig.SetAppConfig("ready", false);
                PaymentModeService.add(popUpData).then((res) => {
                    // console.log(res);
    ...
    const columnNames = [
        { title: "Payment Mode ID", field: "paymentModeId" },
        { title: "Payment Mode Name", field: "paymentModeName" },
