...
import VendorGroupService from "../../services/master/VendorGroupService";
...
import * as VendorLogValidations from "./VendorLog-validator";
...
const VendorLog = (props) => {
    ...
    const getVendorGroupList = async () => {
        var t0 = performance.now();
        await VendorGroupService.fetchAll().then((res) => {
            if (res.data) {
                setVendorGroup(res.data);
    ...
    const dataStruct = {
        fields: {
            vendorLogId: { name: "vendorLogId", dbname: "vendorLogId", componentType: "VIEW", labelDisplayName: "Vendor Log ID", defvalue: "" },
            // vendorLogCode: { name: "vendorLogCode", dbname: "vendorLogCode", componentType: "VIEW", labelDisplayName: "VendorLog Code", defvalue: "" },
            vendorName: { name: "vendorName", dbname: "vendorName", componentType: "INPUT", labelDisplayName: "Vendor Name", defvalue: "", autoFocus: true },
            vendorLoggroup: {
                name: "vendorLoggroup",
                dbname: "vendorLoggroup",
                componentType: "SELECT",
                labelDisplayName: "Vendor Group",
                optionName: "vendorGroupName",
    ...
    const handleEdit = (e, rowData) => {
        ...
        let data={...rowData};
        data["vendorLoggroup"]=utility.getColumnValueasArray(data["vendorLoggroup"],"vendorGroupName")
    ...
    const getFiledValidated = (key, value) => {
        ...
        let rules = VendorLogValidations.getFieldRules(key, popUpData) || {};
    ...
    const handleSave = () => {
        if (validation()) {
            if (popUpMode == "Add" || popUpMode == "Edit") {
                ...
                if (popUpMode == "Add") delete data["vendorLogId"]
                

                data["vendorLoggroup"]=utility.getArrayList({...vendorGroup},"vendorGroupName",data["vendorLoggroup"])
                // console.log(data);
                ...
                console.log(data);
                props.appConfig.SetAppConfig("ready", false);
                VendorLogService.add(popUpData).then((res) => {
                    // console.log(res);
    ...
    const actions = [
        ...
        rowData => ({
            ...
            onClick: (event, rowData) => {
                props.appConfig.SetAppConfig("ready", false);
                VendorLogService.del(rowData["vendorLogId"]).then((res) => {
                    // console.log(res);
                    props.appConfig.openSnackbar("Data Deleted Successfully!!! (" + res.data + ")", "success");
                    AlertService.Success("Data Deleted Successfully!!! (" + res.data + ")");
                    handleRefresh(event);
                    props.appConfig.SetAppConfig("ready", true);
                }).catch((err) => {
                    props.appConfig.openSnackbar(err.response.data.message, "error");
            ...
            hidden: utility.checkUserRights("Delete", props.authList)
        }),
    ...
    const validate = param => {
        ...
        if(param["autotarget"]){
            name=param["autotarget"]["name"]
            value=param["autotarget"]["value"];
        }else{
            value = (type == "checkbox" ? param.target.checked : param.target.value);
        }
        
        if (name == "inActive") value = (value ? "Y" : "N");
        obj[name] = value
        let error = getFiledValidated(name, value)
