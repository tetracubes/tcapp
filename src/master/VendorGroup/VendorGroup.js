...
import * as VendorGroupValidations from "./VendorGroup-validator";
...
const VendorGroup = (props) => {
    ...
    const loadData = async () => {
        ...
        await VendorGroupService.fetchAll().then((res) => {
            if (res.data) {
                ...
                let msg = "Vendor Group Data Loaded Successfully!!!" + `( ${utility.formatTime((performance.now() - t0).toFixed(2))} ms)`;
    ...
    const dataStruct = {
        fields: {
            vendorGroupId: {
                name: "vendorGroupId",
                dbname: "vendorGroupId",
                componentType: "View",
                labelDisplayName: "Vendor Group Id: ",

                // required: true
            },
            vendorGroupName: {
                name: "vendorGroupName",
                dbname: "vendorGroupName",
                componentType: "INPUT",
                labelDisplayName: "Vendor Group Name: ",
                defvalue: "",
                required: true
            },
            inActive: {
    ...
    const getFiledValidated = (key, value) => {
        ...
        let rules = VendorGroupValidations.getFieldRules(key, popUpData) || {};
    ...
    const handleSave = () => {
        if (validation()) {
            if (popUpMode == "Add" || popUpMode == "Edit") {
                let data = popUpData;
                // data["vendorGroupId"]=-1
                if (popUpMode == "Add") delete data["vendorGroupId"]
                ...
                // data["inActive"]="N";
                props.appConfig.SetAppConfig("ready", false);
                VendorGroupService.add(popUpData).then((res) => {
                    // console.log(res);
    ...
    const columnNames = [
        { title: "Vendor Group ID", field: "vendorGroupId" },
        { title: "Vendor Group Name", field: "vendorGroupName" },
