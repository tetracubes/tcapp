...
import TaxService from '../../services/master/TaxService';
...
import * as TaxValidations from "./Tax-validator";
...
const Tax = (props) => {
  ...
  useEffect(() => {
    ...
    const loadData=async ()=>{
        ...
        await TaxService.fetchAll().then((res) => {
            if (res.data) {
                setPageData(res.data);
                let msg = "Tax Data Loaded Successfully!!!" + `( ${utility.formatTime((performance.now() - t0).toFixed(2))} ms)`;
    ...
    const getFiledValidated = (key, value) => {
        ...
        let rules = TaxValidations.getFieldRules(key, popUpData) || {};
    ...
    const handleSave = () => {
        if (validation()) {
            if (popUpMode == "Add" || popUpMode == "Edit") {
                let data = popUpData;
                // data["taxId"]=-1
                if (popUpMode == "Add") delete data["taxId"]
                ...
                // data["inActive"]="N";
                props.appConfig.SetAppConfig("ready", false);
                TaxService.add(popUpData).then((res) => {
                    // console.log(res);
