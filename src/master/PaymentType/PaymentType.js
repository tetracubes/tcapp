...
import * as PaymentTypeValidations from "./PaymentType-validator";
...
const PaymentType = (props) => {
    ...
    const loadData = async () => {
        ...
        await PaymentTypeService.fetchAll().then((res) => {
            if (res.data) {
                ...
                let msg = "Payment Type Data Loaded Successfully!!!" + `( ${utility.formatTime((performance.now() - t0).toFixed(2))} ms)`;
    ...
    const dataStruct = {
        fields: {
            paymentTypeId: {
                name: "paymentTypeId",
                dbname: "paymentTypeId",
                componentType: "View",
                labelDisplayName: "Payment Type Id: ",

                // required: true
            },
            paymentTypeName: {
                name: "paymentTypeName",
                dbname: "paymentTypeName",
                componentType: "INPUT",
                labelDisplayName: "Payment Type Name: ",
                defvalue: "",
                required: true
            },
            inActive: {
    ...
    const getFiledValidated = (key, value) => {
        ...
        let rules = PaymentTypeValidations.getFieldRules(key, popUpData) || {};
    ...
    const handleSave = () => {
        if (validation()) {
            if (popUpMode == "Add" || popUpMode == "Edit") {
                ...
                if (popUpMode == "Add") delete data["paymentTypeId"]
                ...
                // data["inActive"]="N";
                props.appConfig.SetAppConfig("ready", false);
                PaymentTypeService.add(popUpData).then((res) => {
                    // console.log(res);
    ...
    const columnNames = [
        { title: "Payment Type ID", field: "paymentTypeId" },
        { title: "Payment Type Name", field: "paymentTypeName" },
