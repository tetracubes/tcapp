...
import * as ProjectControlValidations from "./ProjectControl-validator";
...
const ProjectControl = (props) => {
    ...
    const loadData = async () => {
        ...
        await ProjectControlService.fetchAll().then((res) => {
            if (res.data) {
                ...
                let msg = "Project Control Data Loaded Successfully!!!" + `( ${utility.formatTime((performance.now() - t0).toFixed(2))} ms)`;
    ...
    const dataStruct = {
        fields: {
            projectControlId: {
                ...
                dbname: "projectControlId",
                componentType: "View",
                labelDisplayName: "Project Control Id: ",

                // required: true
            },
            projectControlName: {
                name: "projectControlName",
                dbname: "projectControlName",
                componentType: "INPUT",
                labelDisplayName: "Project Control Name: ",
                defvalue: "",
                required: true
            },
            inActive: {
    ...
    const getFiledValidated = (key, value) => {
        ...
        let rules = ProjectControlValidations.getFieldRules(key, popUpData) || {};
    ...
    const handleSave = () => {
        if (validation()) {
            if (popUpMode == "Add" || popUpMode == "Edit") {
                let data = popUpData;
                // data["projectControlId"]=-1
                if (popUpMode == "Add") delete data["projectControlId"]
                ...
                // data["inActive"]="N";
                props.appConfig.SetAppConfig("ready", false);
                ProjectControlService.add(popUpData).then((res) => {
                    // console.log(res);
    ...
    const columnNames = [
        { title: "Project Control ID", field: "projectControlId" },
        { title: "Project Control Name", field: "projectControlName" },
