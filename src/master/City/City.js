...
import * as CityValidations from "./City-validator";
...
const City = (props) => {
  ...
  useEffect(() => {
    ...
    const getFiledValidated = (key, value) => {
        ...
        let rules = CityValidations.getFieldRules(key, popUpData) || {};
    ...
    const handleSave = () => {
        if (validation()) {
            if (popUpMode == "Add" || popUpMode == "Edit") {
                let data = popUpData;
                // data["cityId"]=-1
                if (popUpMode == "Add") delete data["cityId"]
                ...
                // data["inActive"]="N";
                props.appConfig.SetAppConfig("ready", false);
                CityService.add(popUpData).then((res) => {
                    // console.log(res);
