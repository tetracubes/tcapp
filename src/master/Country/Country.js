...
import CountryService from '../../services/master/CountryService';
...
import * as CountryValidations from "./Country-validator";
...
const Country = (props) => {
    ...
    const loadData=async ()=>{
        ...
        await CountryService.fetchAll().then((res) => {
            if (res.data) {
                setPageData(res.data);
                let msg = "Country Data Loaded Successfully!!!" + `( ${utility.formatTime((performance.now() - t0).toFixed(2))} ms)`;
    ...
    const getFiledValidated = (key, value) => {
        ...
        let rules = CountryValidations.getFieldRules(key, popUpData) || {};
    ...
    const handleSave = () => {
        if (validation()) {
            if (popUpMode == "Add" || popUpMode == "Edit") {
                let data = popUpData;
                // data["countryId"]=-1
                if (popUpMode == "Add") delete data["countryId"]
                ...
                // data["inActive"]="N";
                props.appConfig.SetAppConfig("ready", false);
                CountryService.add(popUpData).then((res) => {
                    // console.log(res);
    ...
    const columnNames = [
        { title: "Country ID", field: "countryId" },
