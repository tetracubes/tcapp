...
import * as PaymentValidations from "./FinalPayment-validator";
...
const FinalPayment = (props) => {
    ...
    const getActivityList = async (finyear) => {
        ...
        await ActivityService.fetchAllByFinYear(finyear,"Y").then((res) => {
            if (res.data) {
    ...
    const dataStruct = {
        fields: {
            ...
            poamt: { name: "poamt", dbname: "poamt", componentType: "VIEW", type: "number", labelDisplayName: "Payment Amount", defvalue: 0, pattern: "^[0-9]+(.[0-9]{1,5})?$", },
            BalAmt: { name: "BalAmt", dbname: "BalAmt", componentType: "VIEW", type: "number", labelDisplayName: "Payment Amount", defvalue: 0, pattern: "^[0-9]+(.[0-9]{1,5})?$", },
            paymentTypeId: { name: "paymentTypeId", dbname: "paymentTypeId", componentType: "VIEW", labelDisplayName: "Payment Type", defvalue: "Advance Payment", getOptions: getOptions, optionName: "paymentTypeName" },
            payAmt: { name: "payAmt", dbname: "payAmt", componentType: "VIEW", type: "number", labelDisplayName: "Payment Amount", defvalue: 0, pattern: "^[0-9]+(.[0-9]{1,5})?$", },
...
const getPoAmount=(rowData,type)=>{
    ...
    const actions = [
        ...
        rowData => ({
            ...
            disabled:!(rowData["paymentModeId"]==3 || rowData["paymentModeId"]==4 ),
    ...
    const loadFinComponent = () => {
        let dataStruct = {
            ...
            actcode: { name: "actcode", dbname: "actcode", componentType: "AUTOCOMPLETE", labelDisplayName: "Activity Code", getOptions: getOptions, defvalue: "", 
            optionName: "docNo", optionGroupName: "product",additionalnames:["product","venue"] }
        }
        // headerData
        let content = []
